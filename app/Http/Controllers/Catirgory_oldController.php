...
class CatirgoryController extends Controller
{
   ...
   return response()->json(['company' => $employees,]);
    ...
    {           
     ...
     $images = json_decode($empl->images, true);  
    //  $name = json_decode($empl->name, true);  
    //  $name = json_decode($empl->name, true);  
...
// [updated_at] => 2021-07-31 16:28:33
      array_push($stack, array("id"=>$id,"name"=>$name["en"],"address"=>$address["en"],"region"=>$region["en"],"city"=>$city["en"],"lat"=>$lat,"lng"=>$lng,"phones"=>$phones["en"],"images"=>$images));
